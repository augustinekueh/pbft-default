...
//set the delay for recurring transactions
...
func (c* Client) transactionSchedule(ping func(), delay time.Duration)chan bool{
	...
	go func(){//this method is running asynchronously
		for{
			start := time.Now()
			//fmt.Println(start)
			ping()	
			...
			case <- c.timerFlag:
				...
				if !isExist("basic_test_results.txt"){
					err := ioutil.WriteFile("basic_test_results.txt", []byte(write), 0644)
					if err != nil{
						log.Panicf("datalog: error creating file: %s", err)
					}
				}
				f, err :=os.OpenFile("basic_test_results.txt", os.O_APPEND|os.O_WRONLY, 0644)
				if err != nil{
