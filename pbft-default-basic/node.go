...
var newNodeCount int //do i need this??
...
func (n *Node) handlePrePrepare(payload []byte, sig []byte){
	...
	if digest := createDigest(pp.Request); hex.EncodeToString(digest[:]) != pp.Digest{
		...
		//update(29/6)
		fmt.Println("pp:Digest: ", pp.Digest)
...
func (n *Node) handlePrepare(payload []byte, sig []byte){
	...
	fmt.Println("pre.Digest: ", pre.Digest, pre.NodeID)
	msgNodePubKey := getPubKey(pre.NodeID)
	//decode string to byte format 
	digestByte, _ := hex.DecodeString(pre.Digest)
	if !n.verifyRequestDigest(pre.Digest){
		note := fmt.Sprintf("prepare phase: unable to verify (%s) digest, further application rejected!\n", pre.NodeID)
		n.recordPenalties(note)
	} else if n.sequenceID != pre.SequenceID{
		note := fmt.Sprintf("prepare phase: incorrect sequence (%s), further application rejected!\n", pre.NodeID)
		n.recordPenalties(note)
		...
		if n.nodeID == "N0" {
			specifiedCount = nodeCount / 3 * 2 //<--problem with nodeCount
		} else{
...
//update (30/6): from returning err to bool
func (n *Node) verifyRequestDigest(digest string) bool{
	n.mutex.Lock()
	_, ok := n.requestPool[digest]
	if !ok{
		n.mutex.Unlock()
		fmt.Println("verify request digest failed")
		return false
	}
	n.mutex.Unlock()
	return true
